...
2. **mona-lisa-blur.avi** (с размытием):
...
3. **simple_track.avi**:
   - Тестирование на простом объекте.

4. **Собственное видео** (см. папку `inputs`):
   - [Здесь можно добавить описание результатов на собственном видео]
...
- Может давать сбои при сильном размытии (motion blur).

Для задач реального времени на слабых устройствах можно рассмотреть использование дескрипторов ORB или алгоритмов оптического потока (Lucas-Kanade), однако SIFT обеспечивает более высокую надежность при сложных трансформациях объекта.
